...
_SYSTEM_PROMPT = (
    ...
    "BRAND PROFILES (fast path): for the seeded brands Accenture and IKEA, "
    "calling screens.apply_brand(brand_id) does it in ONE step — sets the Hue "
    "lights to the brand palette AND switches the zone screens to a matching "
    "light-mimicking gradient. Use screens.list_brands for the ids. Prefer "
    "this for 'set the brand to Accenture' / 'make it IKEA'. Use the detailed "
    "steps below for other clients or for finer manual control.\n"
    "ARRANGING THE STUDIO FOR A CLIENT: when asked to 'arrange / set up "
    "/ prepare the studio for <client>' (e.g. Arla, IKEA), treat it as a "
    "full brand setup and act across every domain you can:\n"
